        ...
        public static void UseSwaggerMiddleware(this IApplicationBuilder app, IApiVersionDescriptionProvider provider)
        {
            ...
            app.UseSwaggerUI(option =>
            {
                foreach (var item in provider.ApiVersionDescriptions)
                {
                    ...
                    option.SwaggerEndpoint($"/swagger/{item.GroupName}/swagger.json", "HDF.Blog.WebApi API" + item.ApiVersion);
                }
                //option.RoutePrefix = string.Empty;
                option.DocumentTitle = "HDF.Blog API";
            ...
            services.AddSwaggerGen(options =>
            {
                ...
                foreach (var item in provider.ApiVersionDescriptions)
                {
                    ...
                    options.SwaggerDoc(item.GroupName, new OpenApiInfo
                    {
                        ...
                        Title = $"HDF.Blog API {item.ApiVersion}",
                        Description = "API for HDF.Blog",
                        Contact = new OpenApiContact() { Name = "HDF", Email = "1213159982@qq.com", Url = new Uri("https://hdefu.com") }
                    });
                }
